...
void cadastrarVeiculo(Veiculos** vetorVeiculos, int tamanhoVetor) {
	*vetorVeiculos = realloc(*vetorVeiculos, (tamanhoVetor + 1) * sizeof(Veiculos));

	// Lendo os dados
	printf();
	scanf("%s", *vetorVeiculos[tamanhoVetor]->placa);
	printf();
	scanf("%s", *vetorVeiculos[tamanhoVetor]->placa);
	printf();
	scanf("%s", *vetorVeiculos[tamanhoVetor]->placa);
...
int main() {
	...
	while ((escolha = Menu()) != 0) {
		switch (escolha) {
		case 1:
			cadastrarVeiculo(&vetorVeiculos, tamanhoDoVetor);
			break;
		default:
			break;
		}
