...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_replicate", Replicatetoggle, "Toggles client replication");
...
public Action Replicatetoggle(int client, int args)
{
	if(toggleReplicate)
	{
		ReplyToCommand(client, "Toggle off!");
		toggleReplicate = false;
	}
	else
	{
		toggleReplicate = true;
		weapon_accuracy_nospread.Flags &= ~FCVAR_REPLICATED;
		ReplyToCommand(client, "Toggle on!");
	}
...
public void PostThink(int client)
{
    if(IsPlayerAlive(client))
    {
        ...
        {
        	nospread = (velocity[0]*velocity[0]+velocity[1]*velocity[1]) < accuracyThreshold[client]; //70.0^2 - should probably be based off held weapon
       	}
       	else
       	{
       		nospread = false;
