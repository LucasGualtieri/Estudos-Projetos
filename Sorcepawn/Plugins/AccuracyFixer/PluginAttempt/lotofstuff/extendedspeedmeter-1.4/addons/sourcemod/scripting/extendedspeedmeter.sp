...
new Float:g_fPlugin_DisplayTick = 0.000;
...
stock ConnectSQL()
{
	...
	if (SQL_CheckConfig(g_sPlugin_DatabaseConfigName))
	{
		// Database config found, try to connect
		SQL_TConnect(ConnectSQLCallback, g_sPlugin_DatabaseConfigName);
	}
	else
	{
		// No database config found
		SetFailState("%s: Plugin stopped, no config entry found for '%s' in databases.cfg", PLUGIN_NAME, g_sPlugin_DatabaseConfigName);
	}
}
	...
	for (new i=0; i<sessionSize; i++)
	{
		...
		if (GetArrayCell(g_hSessionNewRecord, i) || GetArrayCell(g_hSessionHigherRecord, i))
		{
			...
			else if (GetArrayCell(g_hSessionHigherRecord, i))
			{
				...
				FormatEx(sQuery, sizeof(sQuery), "UPDATE topspeed SET name = '%s', speed = %.2f, timestamp = '%s' WHERE map = '%s' AND auth = '%s';", clientName, topspeed, topspeedTimeStamp, g_sCurrentMap, clientSteamId);
